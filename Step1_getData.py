...
class GetData():
    def __init__(self, base_path=None):
        ...
        if base_path is not None:
            ...
            self.PATH = '/content/drive/MyDrive/TFM/DeepTTC-main'
        elif os.path.exists('/mnt/frareuirod3'):
            # Servidor remoto SSH (ever)
            # Nota: Si los datos de 'DeepTTC-main' los metiste dentro de 'DeepTTA-TFG',
            # la ruta sería '/mnt/frareuirod3/DeepTTA-TFG'
            self.PATH = '/mnt/frareuirod3/DeepTTA-TFG'
        else:
            # Local Windows
            self.PATH = 'C:/Users/franc/Desktop/TFG/DeepTTC-main'
            
        print(f"📁 Ruta de datos configurada en: {self.PATH}")

        rnafile = self.PATH + '/Cell_line_RMA_proc_basalExp.txt'
        smilefile = self.PATH + '/smile_inchi.csv'
        pairfile = self.PATH + '/GDSC_data/GDSC2_fitted_dose_response_25Feb20.xlsx'
        drug_infofile = self.PATH + "/GDSC_data/Drug_listTue_Aug10_2021.csv"
        drug_thred = self.PATH + '/GDSC_data/IC50_thred.txt'
...
#-----------------------------------------------------------------
    def Random95(self, random_seed):
        
        """
        Split 95/5 aleatorio puro (sin estratificar por cáncer ni fármaco).
        Replica la configuración del paper original de DeepTTA (Jiang et al., 2022).
        """
        drug_cell_df = pd.read_excel(self.pairfile)
        self._stat_drug(drug_cell_df)
        self._stat_cell(drug_cell_df)
        ...
        drug_cell_df = drug_cell_df[['DRUG_ID', 'COSMIC_ID', 'TCGA_DESC', 'LN_IC50']]

        # Split 95/5 completamente aleatorio (como en el paper original)
        ...
        print('#' * 50)
        print('#\t Split 95/5 aleatorio (paper original)')
        print('#\t Datos totales: {}'.format(drug_cell_df.shape[0]))
        print('#\t Entrenamiento: {}'.format(train_data.shape[0]))
